...
def main():
    ...
    #     print(np.all(train_raw_target[target_idx[i]]==i))
        ...
        split_point = int(len(val_idx[i])/16)
        val_idx[i] = val_idx[i][:split_point]
        
        train_idx[i] = target_idx[i][split_point:]
        split_point = int(len(train_idx[i])/16)
...
#     mask = np.where(np.isfinite(nTRAIN_ACCRUACY)
    ...
    if args.save_model:
        torch.save(model.state_dict(), "best_mnist_model_16.pt")
